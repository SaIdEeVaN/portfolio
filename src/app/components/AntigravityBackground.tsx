...
export default function AntigravityBackground() {
  ...
  const initParticles = (width: number, height: number) => {
    ...
    for (let i = 0; i < settings.count; i++) {
      ...
      // Slightly more visible, still subtle behind glass UI.
  ...
  useEffect(() => {
    resizeCanvas();
    // Draw immediately so it shows up even before any pointer movement.
    ensureLoop();
    ...
    const onScroll = () => {
      // On some devices users may scroll without triggering pointer events.
      // Kick the loop once so the dots remain visibly present.
      ensureLoop();
    };
